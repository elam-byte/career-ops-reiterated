...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │    └─► claude -p worker → report .md + PDF + tracker entry
  │
  ├─ Offer 2: clicks next, reads JD + URL
  │    └─► claude -p worker → report .md + PDF + tracker entry
  │
  └─ End: merge tracker-additions → applications.jsonl + summary
```
...
batch/
  ...
  tracker-additions/            # JSONL lines per offer (gitignored)
```

## Mode A: Conductor --chrome

1. **Read state**: `batch/batch-state.tsv` → skip already-processed offers
2. **Navigate portal**: Chrome → search results URL
...
6. **End**: Merge `tracker-additions/` → `data/applications.jsonl` + summary
...
- If it dies → re-run → reads `batch-state.tsv` → skips completed entries
...
Each worker receives `batch-prompt.md` as system prompt. Self-contained.

The worker produces:
1. Report `.md` in `reports/`
2. PDF in `output/` (via LaTeX: `python cv/generate_pdf.py --company X --role Y`)
3. JSONL entry in `batch/tracker-additions/{id}.jsonl`
